...
do {
    ...
    switch ($choice) {
        ...
        "6" {
            ...
            $removeVolumes = Read-Host "¿Eliminar también los volúmenes (datos de BD)? (s/n)"
            if ($removeVolumes -eq "s" -Or $removeVolumes -eq "S") {
                docker compose down -v
                Print-Success "Volúmenes eliminados"
            }
        }
    ...
    if ($choice -ne "7") {
        Write-Host ""
        Write-Host "Presiona Enter para volver al menú..." -NoNewline
        $null = Read-Host
    }
